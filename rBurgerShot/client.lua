...
Citizen.CreateThread(function()
    while true do
        local Timer = 500
        local plyPos = GetEntityCoords(PlayerPedId())
        local dist = #(plyPos-Config.posKitchen)
        if ESX.PlayerData.job and ESX.PlayerData.job.name == 'burgershot' then
        if dist <= 10.0 then
         Timer = 0
         DrawMarker(6,  Config.posKitchen.x, Config.posKitchen.y, Config.posKitchen.z-0.99, nil, nil, nil, -90, nil, nil, 1.0, 1.0, 1.0, 255, 0, 0 , 120)
        end
         if dist <= 3.0 then
            Timer = 0
